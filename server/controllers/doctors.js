...
module.exports = function(router){
    router.get('/', function (req, res) {
        User.find({$and: [{doctorData: { $ne: null }}, {"doctorData.isDeleted": {$ne: true}}]},function(err, doctor){
            res.send(doctor);
        });
    });
    router.put('/:id', function(req,res){
        ...
        if(typeof req.body.delete != 'undefined' && req.body.delete === true){
            ...
            User.findById(req.params.id, function (err, doctor) {
                ...
                doctor.password = (req.body.password)?passwordHash.generate(req.body.password, {algorithm: config.hashAlgorithm}):doctor.password;
